...
import { Play, Trash2, Download, Pencil, AlertTriangle } from "lucide-react";
  ...
  const unifiedItems = useMemo<UnifiedItem[]>(() => {
    ...
    const server: UnifiedItem[] = serverItems
      ...
      .map((item) => ({
        ...
        lyrics: item.lyrics,
        styleTags: item.styleTags,
        coverImageUrl: item.coverImagePath
          ? `/api/music/history?id=${item.id}&type=cover`
          : undefined,
    ...
    <div className="space-y-2">
      {!isDev && unifiedItems.length > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-sm text-amber-400">
          ...
          <span>{t("production.downloadWarning")}</span>
        </div>
      )}
